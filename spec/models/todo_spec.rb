...
RSpec.describe Todo, type: :model do
  ...
  context 'validation tests' do
    ...
    it 'ensure task presence' do
      todo = Todo.new(user_id: 2, due_date: "2022-10-12", is_done: false).save
      expect(todo).to eq(false)
    end 
    it 'ensure due date presence' do
      todo = Todo.new(user_id: 2, task: "test the code", is_done: false).save
      expect(todo).to eq(false)
    end
    it 'todo should save successfully' do
      todo = Todo.new(user_id: 2, task: "test the code", due_date: "2022-10-12", is_done: 0).save
      expect(todo).to eq(true)
    end
    context 'scope tests' do
      let (:params) { {user_id:2, task: "test the code"} }
      before(:each) do
